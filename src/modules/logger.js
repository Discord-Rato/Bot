    ...
    if (type === "log") {
        ...
        if (process.argv.includes("--dev") === false) {
            try {
                ...
                console.log(
                    `[Sentry Error] | Failed to capture exception warning (${content}) to Sentry. ${err}`
                );
            }
        }
        ...
        if (process.argv.includes("--dev") === false) {
            try {
                ...
                console.log(
                    `[ERROR] Failed to capture exception (${content}) to Sentry. ${err}`
                );
            }

            if (global?.bot?.isReady() === true) {
                const errorChannel = await global.bot.channels.fetch(
                    "948686231892545547"
                );
...
/**
 * @INFO
 * ©️ Ch1ll Studios, https://discord.gg/kHFknhsr
 */
