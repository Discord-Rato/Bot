...
module.exports = class BalanceCommand extends BaseCommand {
    constructor() {
        super("daily", "Earn a salary every day.", "economy", [], 86400000);
    }
    ...
    async run({ client, interaction, args }) {
        ...
        SavedUser.findOne({ _id: user.id }, async (err, data) => {
            ...
            data.daily_streeks += 1;
            data.transactions.push({
                action: "daily",
                type: `+`,
                description: `${parseInt(
                    amount
                ).toLocaleString()} for collecting daily reward.`,
            });
            data.coins += amount;
            await data.save();
            ...
            const embed = new MessageEmbed()
                ...
                .setDescription(
                    `> ${client.success}  **You claimed your :coin: ${parseInt(
                        amount
                    )?.toLocaleString()} for the day come back tomorrow!**`
                )
                .setColor("BLURPLE");
